...
let guestName;
function showGreeting(guestName) {
...
const total = calculateProduct(5, 3); //Выведет 15
...
let calculateSquare = (num) => num * num;
...
console.log('Вашему другу сейчас ' + calculateAge(currentYear, friendYearOfBirth) + " лет.");
...
const myYearOfBirth = 1985;

let myAge = (currentYear, myYearOfBirth) => {
	console.log("Мой возраст " + `${currentYear - myYearOfBirth}` + " лет.");
}

myAge(currentYear, myYearOfBirth);
...
let z = (a, x) => {
	let result = a ** x;
	console.log(`Результат возведения ${a} в степень ${x}: ${result}`);
};
z(3, 4);